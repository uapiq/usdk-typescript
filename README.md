...
```sh
npm install git+ssh://git@github.com:uapiq/usdk-typescript.git
```
...
const response = await client.search({ query: 'When is the next solar eclipse?' });
```
...
const params: uAPI.ExtractParams = { url: 'https://finance.yahoo.com/quote/NVDA/' };
const response: unknown = await client.extract(params);
```
...
await client.extract({ url: 'https://finance.yahoo.com/quote/NVDA/' }, {
  maxRetries: 5,
});
```
...
1. Via the `UAPI_LOG` environment variable
